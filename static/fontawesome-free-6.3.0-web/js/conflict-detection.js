      ...
      function poll(duration, cumulativeDuration) {
        setTimeout(function () {
          ...
          if (!!result) {
            ...
            if (nextCumulativeDuration <= maxduration) { poll(nextduration, nextcumulativeduration); } else reject('timeout'); eslint-disable-line prefer-promise-reject-errors }, duration); poll(initialduration, 0); }); function detectwebfontconflicts() var linktags="Array.from(DOCUMENT.getElementsByTagName('link')).filter(function" (t) return !t.hasattribute(detectionignoreattr); styletags="Array.from(DOCUMENT.getElementsByTagName('style')).filter(function" if (t.hasattribute(detectionignoreattr)) false; the browser has loaded fa5 css, let's not test that <style> element.
